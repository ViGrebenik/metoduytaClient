export const mockData = [
	{
		id: 1, // id
		type: 'Комплексный ремонт', // тип ремонта
		apartmentType: 'Новостройка',
		description:
			'Прекрасно обновленная квартира площадью 44m2, после капитального ремонта под ключ. Интерьер выполнен в светлых тонах, создавая атмосферу простора и света.', // короткое описание
		listWorks: [
			['Электромонтажные работы (черновые и чистовые)'],
			['Сантехнические работы (черновые и чистовые)'],
			['Грунтовка стен'],
			['Шпатлевка стен'],
			['Ошкуривание'],
			['Оклейка стен обоями'],
			['Монтаж теплого пола'],
			['Устройство финишной стяжки'],
			['Укладка ламината на подложку'],
			['Установка подоконников'],
			['Монтаж откосов оконных и дверных под окраску'],
			['Установка плинтуса'],
			['Укладка керамогранита на пол с затиркой швов']
		],
		area: 44, // площадь квартиры
		price: 697.556, // цена ремонта
		priceArea: 15.8,
		timeWork: 3,
		location: 'Пр.Героев 35',
		photos: [
			'/static/archive/MocPortfolio/object_01/item_01.jpeg',
			'/static/archive/MocPortfolio/object_01/item_02.jpeg',
			'/static/archive/MocPortfolio/object_01/item_03.jpeg',
			'/static/archive/MocPortfolio/object_01/item_04.jpeg',
			'/static/archive/MocPortfolio/object_01/item_05.jpeg',
			'/static/archive/MocPortfolio/object_01/item_06.jpeg',
			'/static/archive/MocPortfolio/object_01/item_07.jpeg'
		]
	},
	{
		id: 2, // id
		type: 'Комплексный ремонт', // тип ремонта
		apartmentType: 'Новостройка',
		description:
			'Обновленная однокомнатная квартира в современном стиле после комплексного ремонта под ключ', // короткое описание
		listWorks: [
			['Электромонтажные работы (черновые и чистовые)'],
			['Сантехнические работы (черновые и чистовые)'],
			['Грунтовка стен и потолка'],
			['Шпатлевка стен и потолка'],
			['Ошкуривание стен и потолка под окраску'],
			['Окраска стен и потолка'],
			['Монтаж теплого пола'],
			['Устройство финишной стяжки'],
			['Укладка ламината на подложку'],
			['Установка подоконников'],
			['Монтаж откосов оконных и дверных под окраску'],
			['Установка плинтуса'],
			['Укладка керамогранита на пол с затиркой швов']
		],
		area: 58, // площадь квартиры
		price: 890.56, // цена ремонта
		priceArea: 15.3,
		timeWork: 3.5,
		location: 'ВО 5 линия ',
		photos: [
			'/static/archive/MocPortfolio/object_02/item_01.jpeg',
			'/static/archive/MocPortfolio/object_02/item_02.jpeg',
			'/static/archive/MocPortfolio/object_02/item_03.jpeg',
			'/static/archive/MocPortfolio/object_02/item_04.jpeg',
			'/static/archive/MocPortfolio/object_02/item_05.jpeg',
			'/static/archive/MocPortfolio/object_02/item_06.jpeg',
			'/static/archive/MocPortfolio/object_02/item_07.jpeg'
		]
	},
	{
		id: 3, // id
		type: 'Дизайнерский ремонт', // тип ремонта
		apartmentType: 'Вторичка',
		description:
			'Однокомнатная квартира после дизайнерского ремонта с интересными решениями на балконе и светлыми интерьерными решениями', // короткое описание
		listWorks: [
			['Электромонтажные работы (черновые и чистовые)'],
			['Сантехнические работы (черновые и чистовые)'],
			['Монтаж трапа с душ кабиной'],
			['Грунтовка стен'],
			['Шпатлевка стен'],
			['Ошкуривание под окраску'],
			['Окраска стен'],
			['Утепление лоджии (стены, пол и потолок)'],
			['Монтаж теплого пола'],
			['Устройство финишной стяжки'],
			['Укладка кварцвиниловой плитки на подложку'],
			['Установка подоконников'],
			['Монтаж откосов оконных и дверных под окраску'],
			['Установка плинтуса'],
			['Укладка керамогранита на пол с затиркой швов']
		],
		area: 46, // площадь квартиры
		price: 723.59, // цена ремонта
		priceArea: 15.7,
		location: 'Пр.Заневский 42',
		timeWork: 3.5,
		photos: [
			'/static/archive/MocPortfolio/object_03/item_01.jpeg',
			'/static/archive/MocPortfolio/object_03/item_02.jpeg',
			'/static/archive/MocPortfolio/object_03/item_03.jpeg',
			'/static/archive/MocPortfolio/object_03/item_04.jpeg',
			'/static/archive/MocPortfolio/object_03/item_05.jpeg',
			'/static/archive/MocPortfolio/object_03/item_06.jpeg'
		]
	},
	{
		id: 4, // id
		type: 'Комплексный ремонт', // тип ремонта
		// тип квартиры
		apartmentType: 'Новостройка',
		description:
			'Студия совмещена с балконом, что придало ей дополнительное пространство и уникальный характер', // короткое описание
		listWorks: [
			['Электромонтажные работы (черновые и чистовые)'],
			['Сантехнические работы (черновые и чистовые)'],
			['Монтаж душевой кабины с трапом'],
			['Грунтовка стен'],
			['Шпатлевка стен'],
			['Ошкуривание под окраску'],
			['Окраска стен'],
			['Утепление лоджии (стены, пол и потолок)'],
			['Монтаж теплого пола'],
			['Укладка керамогранита на пол с затиркой швов'],
			['Монтаж натяжного потолка со встроенными светильниками'],
			['Устройство закарнизной ниши и установка карнизов']
		],
		area: 33, // площадь квартиры
		price: 762.41, // цена ремонта
		priceArea: 23.1,
		timeWork: 3,
		location: 'Ул.Беринга 27',
		photos: [
			'/static/archive/MocPortfolio/object_04/item_01.jpeg',
			'/static/archive/MocPortfolio/object_04/item_02.jpeg',
			'/static/archive/MocPortfolio/object_04/item_03.jpeg',
			'/static/archive/MocPortfolio/object_04/item_04.jpeg',
			'/static/archive/MocPortfolio/object_04/item_05.jpeg',
			'/static/archive/MocPortfolio/object_04/item_06.jpeg',
			'/static/archive/MocPortfolio/object_04/item_07.jpeg',
			'/static/archive/MocPortfolio/object_04/item_08.jpeg',
			'/static/archive/MocPortfolio/object_04/item_09.jpeg'
		]
	},
	{
		id: 5, // id
		type: 'Дизайнерский ремонт', // тип ремонта
		// тип квартиры
		apartmentType: 'Вторичка',
		description:
			'Просторная двухкомнатная квартира с выразительными дизайнерскими находками в коридоре и на кухне, добавляющими яркие акценты и уникальный стиль.', // короткое описание
		listWorks: [
			['Электромонтажные работы (черновые и чистовые)'],
			['Сантехнические работы (черновые и чистовые)'],
			['Монтаж душевой кабины с трапом'],
			['Грунтовка стен'],
			['Шпатлевка стен'],
			['Ошкуривание под окраску'],
			['Окраска стен'],
			['Утепление лоджии (стены, пол и потолок)'],
			['Монтаж теплого пола'],
			['Устройство финишной стяжки'],
			['Укладка кварцвиниловой плитки на подложку'],
			['Установка подоконников'],
			['Монтаж откосов оконных и дверных под окраску'],
			['Установка плинтуса'],
			['Укладка керамогранита на пол с затиркой швов'],
			['Монтаж натяжного потолка со встроенными светильниками'],
			['Устройство закарнизной ниши и установка карнизов']
		],
		area: 71, // площадь квартиры
		price: 1111.24, // цена ремонта
		priceArea: 15.5,
		timeWork: 3.5,
		location: 'Ш.Лаврики 72',
		photos: [
			'/static/archive/MocPortfolio/object_05/item_01.jpeg',
			'/static/archive/MocPortfolio/object_05/item_02.jpeg',
			'/static/archive/MocPortfolio/object_05/item_03.jpeg',
			'/static/archive/MocPortfolio/object_05/item_04.jpeg',
			'/static/archive/MocPortfolio/object_05/item_05.jpeg',
			'/static/archive/MocPortfolio/object_05/item_06.jpeg',
			'/static/archive/MocPortfolio/object_05/item_07.jpeg',
			'/static/archive/MocPortfolio/object_05/item_08.jpeg',
			'/static/archive/MocPortfolio/object_05/item_09.jpeg'
		]
	},
	{
		id: 6, // id
		type: 'Косметический', // тип ремонта
		// тип квартиры
		apartmentType: 'Вторичка',
		description: 'Косметический ремонт на кухне и ванной компанте', // короткое описание
		listWorks: [
			['Электромонтажные работы (черновые и чистовые)'],
			['Сантехнические работы (черновые и чистовые)'],
			['Монтаж душевой кабины с трапом'],
			['Грунтовка стен'],
			['Шпатлевка стен'],
			['Ошкуривание'],
			['Оклейка стен обоями'],
			['Укладка ламината на подложку'],
			['Установка подоконников'],
			['Монтаж откосов оконных и дверных под окраску'],
			['Установка плинтуса'],
			['Укладка керамогранита на пол с затиркой швов']
		],
		area: 34, // площадь квартиры
		price: 337.882, // цена ремонта
		priceArea: 9.8,
		timeWork: 1.5,
		location: 'П.Комендантский  63',
		photos: [
			'/static/archive/MocPortfolio/object_06/item_01.jpeg',
			'/static/archive/MocPortfolio/object_06/item_02.jpeg',
			'/static/archive/MocPortfolio/object_06/item_03.jpeg',
			'/static/archive/MocPortfolio/object_06/item_04.jpeg',
			'/static/archive/MocPortfolio/object_06/item_05.jpeg'
		]
	},
	{
		id: 7, // id
		type: 'Комплексный ремонт', // тип ремонта
		// тип квартиры
		apartmentType: 'Новостройка',
		description: 'Комплексный ремонт в центре города в новом современном ЖК', // короткое описание
		listWorks: [
			['Электромонтажные работы (черновые и чистовые)'],
			['Сантехнические работы (черновые и чистовые)'],
			['Установка ванны с экраном'],
			['Грунтовка стен'],
			['Шпатлевка стен'],
			['Ошкуривание под окраску'],
			['Окраска стен'],
			['Утепление лоджии (стены, пол и потолок)'],
			['Монтаж теплого пола'],
			['Устройство финишной стяжки'],
			['Укладка кварцвиниловой плитки на подложку'],
			['Установка подоконников'],
			['Монтаж откосов оконных и дверных под окраску'],
			['Установка плинтуса'],
			['Укладка керамогранита на пол с затиркой швов'],
			['Монтаж натяжного потолка со встроенными светильниками'],
			['Устройство закарнизной ниши и установка карнизов']
		],
		area: 45, // площадь квартиры
		price: 710.781, // цена ремонта
		priceArea: 15.8,
		timeWork: 2,
		location: 'Ул.Пионерская 32',
		photos: [
			'/static/archive/MocPortfolio/object_07/item_01.jpeg',
			'/static/archive/MocPortfolio/object_07/item_02.jpeg',
			'/static/archive/MocPortfolio/object_07/item_03.jpeg',
			'/static/archive/MocPortfolio/object_07/item_04.jpeg',
			'/static/archive/MocPortfolio/object_07/item_05.jpeg',
			'/static/archive/MocPortfolio/object_07/item_06.jpeg',
			'/static/archive/MocPortfolio/object_07/item_07.jpeg'
		]
	},
	{
		id: 8, // id
		type: 'Комплексный ремонт', // тип ремонта
		// тип квартиры
		apartmentType: 'Вторичка',
		description: 'Комплексный ремонт под ключ', // короткое описание
		listWorks: [
			['штукатурка стен по маякам'],
			['шпаклёвка стен под покраску'],
			['покраска стен'],
			['сантехнические работы'],
			['электромонтажные работы'],
			['сборка нового электрощита']
		],
		area: 89, // площадь квартиры
		price: 990.29, // цена ремонта
		priceArea: 11.1,
		timeWork: 3,
		location: 'Б.Измайловский 4',
		photos: [
			'/static/archive/MocPortfolio/object_08/item_01.jpeg',
			'/static/archive/MocPortfolio/object_08/item_02.jpeg',
			'/static/archive/MocPortfolio/object_08/item_03.jpeg',
			'/static/archive/MocPortfolio/object_08/item_04.jpeg',
			'/static/archive/MocPortfolio/object_08/item_05.jpeg',
			'/static/archive/MocPortfolio/object_08/item_06.jpeg',
			'/static/archive/MocPortfolio/object_08/item_07.jpeg'
		]
	},
	{
		id: 9, // id
		type: 'Косметический', // тип ремонта
		// тип квартиры
		apartmentType: 'Вторичка',
		description:
			'Освежающий косметический ремонт в ванной комнате, зале и на балконе', // короткое описание
		listWorks: [
			['Электромонтажные работы (черновые и чистовые)'],
			['Сантехнические работы (черновые и чистовые)'],
			['Монтаж душевой кабины с трапом'],
			['Грунтовка стен'],
			['Шпатлевка стен'],
			['Ошкуривание под окраску'],
			['Окраска стен'],
			['Оклейка стен обоями'],
			['Монтаж теплого пола'],
			['Устройство финишной стяжки'],
			['Укладка ламината на подложку'],
			['Установка подоконников'],
			['Монтаж откосов оконных и дверных под окраску'],
			['Установка плинтуса'],
			['Укладка керамогранита на пол с затиркой швов']
		],
		area: 73, // площадь квартиры
		price: 965.0, // цена ремонта
		priceArea: 13.2,
		timeWork: 3,
		location: 'Ул.Варшавская 19к5',
		photos: [
			'/static/archive/MocPortfolio/object_09/item_01.jpeg',
			'/static/archive/MocPortfolio/object_09/item_02.jpeg',
			'/static/archive/MocPortfolio/object_09/item_03.jpeg',
			'/static/archive/MocPortfolio/object_09/item_04.jpeg',
			'/static/archive/MocPortfolio/object_09/item_05.jpeg'
		]
	}
]
